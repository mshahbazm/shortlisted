import type { tLocale } from '../en'
import { plural } from './plural'

export const profile: tLocale['profile'] = {
  yourProfile: 'Twój profil',
  hint: 'Wszystko, co rozszerzenie wypełnia, bierze się stąd.',

  city: 'Miasto',
  countryIso: 'Kraj (ISO-2)',
  headline: 'Nagłówek',
  headlinePlaceholder: 'np. AI Agent Engineer',
  summary: 'Podsumowanie',
  skills: 'Umiejętności',
  industries: 'Branże',
  locationPlaceholder: 'Miasto, kraj',

  linksTitle: 'Linki',
  savedToast: 'Zapisano ✓',
  invalidUrl: 'Ten link nie wygląda na poprawny — sprawdź go i spróbuj ponownie.',
  website: 'Strona WWW',
  github: 'GitHub',
  linkedin: 'LinkedIn',
  portfolio: 'Portfolio',

  workTitle: 'Doświadczenie zawodowe',
  roleTitle: 'Stanowisko',
  company: 'Firma',
  fromYm: 'Od (RRRR-MM lub RRRR)',
  toYm: 'Do (puste = obecnie)',
  techUsed: 'Technologie w tej roli (po przecinku)',
  workHighlights: 'Osiągnięcia — jedno na linię, prawdziwe i konkretne',
  untitled: 'Bez nazwy',

  educationTitle: 'Wykształcenie',
  degree: 'Tytuł / stopień',
  fieldOfStudy: 'Kierunek',
  school: 'Uczelnia',
  fromYear: 'Od (rok)',
  toYear: 'Do (rok)',

  standardAnswersTitle: 'Standardowe odpowiedzi',
  answeredOf: (filled: number, total: number) => `uzupełniono ${filled} z ${total}`,
  standardAnswersHint: 'Pytają o to w prawie każdej aplikacji — odpowiedz raz tutaj.',
  salaryHourly: 'Stawka godzinowa',
  salaryMonthly: 'Wynagrodzenie miesięczne',
  noticeDays: 'Wypowiedzenie (dni)',
  noticeDaysHint: '0 = od zaraz',
  yearsOfExperience: 'Lata doświadczenia',
  yearsPlaceholder: 'puste = policzy się z historii pracy',
  timezone: 'Strefa czasowa',
  timezonePlaceholder: '„PKT (UTC+5)”',
  visaSponsorship: 'Sponsorowanie wizy?',
  visaPlaceholder: '„Nie — zdalny kontraktor”',
  authorizedIn: 'Pozwolenie na pracę w',
  relocation: 'Relokacja',
  hoursOverlap: 'Wspólne godziny pracy',
  hoursPlaceholder: '„4+ godz. z US East”',
  englishLevel: 'Poziom angielskiego',

  tellMeTitle: 'Zaktualizuj profil',
  tellMeSummary: 'nowa umiejętność, certyfikat, coś pominiętego',
  tellMePlaceholder: 'Cokolwiek o Tobie — świeżo zdobyta umiejętność, uzyskany certyfikat, język, którym mówisz, albo coś z dawnej pracy, czego nigdy nie zapisałeś. Zwykłe słowa.',
  tellMeButton: 'Dodaj do profilu',
  tellMeFinishJob: (company: string) =>
    `Dodaj stanowisko i daty dla ${company}, aby dopasowane CV nie pokazywało luki.`,
  workNeedsDetail:
    'Brakuje stanowiska i daty rozpoczęcia — dopasowane CV odczyta to jako lukę.',
  tellMeNoSuchJob:
    'Nie udało się powiązać tego z żadną pracą — najpierw dodaj tę pracę w sekcji Doświadczenie, a potem mi o niej powiedz.',
  tellMeNothing: 'Nic nowego — Twój profil już to obejmuje.',
  tellMeAdded: (n: number) => `Zapisano w profilu ✓ (nowe: ${n})`,
  reimportTitle: 'Zaimportuj ponownie z CV',
  uploadPdf: 'Prześlij PDF',
  readingPdf: 'Czytam PDF…',
  reading: 'Czytam…',
  pasteCvPlaceholder: '…albo wklej tekst swojego CV.',
  rebuildProfile: 'Odbuduj profil',
  strengthTitle: 'Siła profilu',
  gapName: 'Brakuje imienia',
  gapContact: 'Brakuje emaila i lokalizacji',
  gapHeadline: 'Brakuje nagłówka i podsumowania',
  gapWorkDates: 'Stanowisko bez dat',
  gapWorkHighlights: 'Ostatnie stanowisko bez osiągnięć',
  gapEducation: 'Brakuje wykształcenia',
  gapSkills: 'Dodaj kilka umiejętności',
  gapAnswers: 'Mało standardowych odpowiedzi',
  whatEmployersAsk: 'O co pytają pracodawcy',
  standardAnswersSub: 'Wynagrodzenie, wypowiedzenie, wiza, przeprowadzka…',
  answerBankTitle: 'Baza odpowiedzi',
  aboutYou: 'O Tobie',
  careerHighlights: 'Najważniejsze osiągnięcia',
  skillsTitle: 'Umiejętności',
  languagesTitle: 'Języki',
  certificationsTitle: 'Certyfikaty',
  reimportBody: 'Wgraj PDF albo wklej tekst, a odbuduję ten profil. Zapisane odpowiedzi zostają.',
  reimportChooseBody: 'Jak wykorzystać to CV?',
  reimportMergeTitle: 'Dowiedz się o mnie więcej',
  reimportMergeSub: 'Dodaje nowości z tego CV — zachowuje wszystko, co już masz.',
  reimportReplaceTitle: 'Zastąp mój profil',
  reimportReplaceSub: 'Użyj tego CV jako profilu — zastępuje to, co jest teraz.',
  reimportReviewTitle: 'Czy to się zgadza?',
  reimportReviewBody: (roles: number, skills: number) =>
    `${roles} stanowisk${roles === 1 ? 'o' : 'a'} i ${skills} umiejętności. Nic nie zapisze się, dopóki nie potwierdzisz.`,
  reimportConfirm: 'Zapisz w profilu',
  reimportMergeReviewBody: (count: number) =>
    `Znaleziono ${count === 1 ? '1 nowy element' : `${count} nowych elementów`} w tym CV. Usuń, czego nie chcesz, i dodaj resztę.`,
  reimportNothingNew: 'Nic nowego w tym CV — Twój profil już to zawiera.',
  reimportNewHighlights: 'Więcej o Twoich stanowiskach',
  linkOther: 'Inne',
  sex: 'Płeć',
  sexMale: 'Mężczyzna',
  sexFemale: 'Kobieta',
  sexOther: 'Inna',
  drivingLicence: 'Prawo jazdy',
  personalDetails: 'Dane osobowe',
  additionalInfo: 'Dodatkowe informacje',
  communicationSkills: 'Umiejętności komunikacyjne',
  organisationalSkills: 'Umiejętności organizacyjne',
  digitalSkills: 'Umiejętności cyfrowe',
  dsInfoProcessing: 'Przetwarzanie informacji',
  dsCommunication: 'Komunikacja',
  dsContentCreation: 'Tworzenie treści',
  dsSafety: 'Bezpieczeństwo',
  dsProblemSolving: 'Rozwiązywanie problemów',
  jobType: 'Typ pracy',
  reimportMergeButton: 'Dodaj do profilu',
  moreCount: 'więcej',
  nothingYet: 'Jeszcze nic tutaj',
  oneCredit: '1 kredyt',
  segProfile: 'Profil',
  segUnanswered: 'Bez odpowiedzi',
  newRole: 'Nowe stanowisko',
  newEducation: 'Nowe wykształcenie',
  addRoleShort: 'Dodaj stanowisko',
  addEducationShort: 'Dodaj wykształcenie',
  removeRoleTitle: 'Usunąć to stanowisko?',
  removeEducationTitle: 'Usunąć to wykształcenie?',
  removeWarning: 'Tego nie da się cofnąć — trzeba by wpisać wszystko od nowa. Gotowe CV zachowują to, z czego powstały.',
  addHighlight: 'Dodaj osiągnięcie',
  addLanguage: 'Dodaj język',
  addCertification: 'Dodaj certyfikat',
  removeItem: 'Usuń',
  skillPlaceholder: 'Wpisz umiejętność i naciśnij Enter',
  industryPlaceholder: 'Wpisz branżę i naciśnij Enter',
  techPlaceholder: 'Wpisz narzędzie i naciśnij Enter',
  highlightPlaceholder: 'Co zrobiłeś i co się dzięki temu zmieniło',
  languageName: 'Język',
  certName: 'Certyfikat',
  issuer: 'Wydany przez',
  yearLabel: 'Rok',
  clearLink: 'Wyczyść',
  lvlElementary: 'Podstawowy',
  lvlLimited: 'Konwersacyjny',
  lvlProfessional: 'Zawodowy',
  lvlFull: 'Biegły',
  lvlNative: 'Ojczysty',
  photo: "Zdjęcie",
  addPhoto: "Dodaj zdjęcie",
  changePhoto: "Zmień zdjęcie",
  photoHint: "Do CV Europass i kontynentalnych.",
}
