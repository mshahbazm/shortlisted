import type { tLocale } from '../en'

export const profile: tLocale['profile'] = {
  yourProfile: 'Tu perfil',
  hint: 'Todo lo que la extensión rellena sale de aquí.',

  city: 'Ciudad',
  countryIso: 'País (ISO-2)',
  headline: 'Titular',
  headlinePlaceholder: 'p. ej. Ingeniero de Agentes de IA',
  summary: 'Resumen',
  skills: 'Habilidades',
  industries: 'Sectores',
  locationPlaceholder: 'Ciudad, País',

  linksTitle: 'Enlaces',
  savedToast: 'Guardado ✓',
  invalidUrl: 'Ese enlace no parece válido — revísalo e inténtalo de nuevo.',
  website: 'Sitio web',
  github: 'GitHub',
  linkedin: 'LinkedIn',
  portfolio: 'Portafolio',

  workTitle: 'Experiencia laboral',
  roleTitle: 'Puesto',
  company: 'Empresa',
  fromYm: 'Desde (AAAA-MM o AAAA)',
  toYm: 'Hasta (vacío = actual)',
  techUsed: 'Tecnologías usadas en este puesto (separadas por comas)',
  workHighlights: 'Logros — uno por línea, reales y concretos',
  untitled: 'Sin título',

  educationTitle: 'Educación',
  degree: 'Título',
  fieldOfStudy: 'Área de estudio',
  school: 'Institución',
  fromYear: 'Desde (año)',
  toYear: 'Hasta (año)',

  standardAnswersTitle: 'Respuestas estándar',
  answeredOf: (filled: number, total: number) => `${filled} de ${total} respondidas`,
  standardAnswersHint: 'Se preguntan en casi toda solicitud — respóndelas una vez aquí.',
  salaryHourly: 'Tarifa por hora',
  salaryMonthly: 'Salario mensual',
  noticeDays: 'Preaviso (días)',
  noticeDaysHint: '0 = disponible de inmediato',
  yearsOfExperience: 'Años de experiencia',
  yearsPlaceholder: 'si lo dejas vacío, se calcula del historial laboral',
  timezone: 'Zona horaria',
  timezonePlaceholder: '"PKT (UTC+5)"',
  visaSponsorship: '¿Patrocinio de visa?',
  visaPlaceholder: '"No — contratista remoto"',
  authorizedIn: 'Autorizado a trabajar en',
  relocation: 'Reubicación',
  hoursOverlap: 'Horas de coincidencia',
  hoursPlaceholder: '"4+ horas con la costa este de EE. UU."',
  englishLevel: 'Nivel de inglés',

  tellMeTitle: 'Actualizar perfil',
  tellMeSummary: 'una habilidad nueva, un certificado, algo que olvidaste',
  tellMePlaceholder: 'Cualquier cosa sobre ti — una habilidad que acabas de aprender, un certificado que obtuviste, un idioma que hablas o algo de un trabajo antiguo que nunca anotaste. Con palabras sencillas.',
  tellMeButton: 'Añadir al perfil',
  tellMeFinishJob: (company: string) =>
    `Añade el puesto y las fechas de ${company} para que un CV adaptado no muestre un hueco.`,
  workNeedsDetail:
    'Faltan el puesto y la fecha de inicio: un CV adaptado lo interpreta como un vacío.',
  tellMeNoSuchJob:
    'No pude vincular eso a un empleo: añade primero el empleo en Experiencia y luego cuéntamelo.',
  tellMeNothing: 'No hay nada nuevo — tu perfil ya lo cubre.',
  tellMeAdded: (n: number) => `Guardado en tu perfil ✓ (${n} nuevo/s)`,
  reimportTitle: 'Reimportar desde el CV',
  uploadPdf: 'Subir PDF',
  readingPdf: 'Leyendo PDF…',
  reading: 'Leyendo…',
  pasteCvPlaceholder: '…o pega el texto de tu CV.',
  rebuildProfile: 'Reconstruir perfil',
  strengthTitle: 'Fuerza del perfil',
  gapName: 'Falta tu nombre',
  gapContact: 'Falta correo y ubicación',
  gapHeadline: 'Falta titular y resumen',
  gapWorkDates: 'Un puesto sin fechas',
  gapWorkHighlights: 'Tu puesto actual necesita logros',
  gapEducation: 'Falta tu formación',
  gapSkills: 'Añade algunas aptitudes más',
  gapAnswers: 'Respuestas estándar incompletas',
  whatEmployersAsk: 'Lo que preguntan las empresas',
  standardAnswersSub: 'Salario, preaviso, visado, mudanza…',
  answerBankTitle: 'Banco de respuestas',
  aboutYou: 'Sobre ti',
  careerHighlights: 'Logros destacados',
  skillsTitle: 'Aptitudes',
  languagesTitle: 'Idiomas',
  certificationsTitle: 'Certificaciones',
  reimportBody: 'Sube un PDF o pega el texto y reconstruyo este perfil. Tus respuestas guardadas se quedan.',
  reimportChooseBody: '¿Cómo usamos este CV?',
  reimportMergeTitle: 'Saber más sobre mí',
  reimportMergeSub: 'Añade lo nuevo de este CV y conserva todo lo que ya tienes.',
  reimportReplaceTitle: 'Reemplazar mi perfil',
  reimportReplaceSub: 'Usa este CV como mi perfil: reemplaza lo que hay ahora.',
  reimportReviewTitle: '¿Se ve bien?',
  reimportReviewBody: (roles: number, skills: number) =>
    `${roles} puesto${roles === 1 ? '' : 's'} y ${skills} habilidades. No se guarda nada hasta que confirmes.`,
  reimportConfirm: 'Guardar en el perfil',
  reimportMergeReviewBody: (count: number) =>
    `${count === 1 ? '1 elemento nuevo' : `${count} elementos nuevos`} en este CV. Quita lo que no quieras y añade el resto.`,
  reimportNothingNew: 'Nada nuevo en este CV: tu perfil ya lo incluye.',
  reimportNewHighlights: 'Más sobre tus puestos',
  linkOther: 'Otro',
  sex: 'Sexo',
  sexMale: 'Masculino',
  sexFemale: 'Femenino',
  sexOther: 'Otro',
  drivingLicence: 'Carné de conducir',
  personalDetails: 'Datos personales',
  additionalInfo: 'Información adicional',
  communicationSkills: 'Habilidades de comunicación',
  organisationalSkills: 'Habilidades organizativas',
  digitalSkills: 'Competencias digitales',
  dsInfoProcessing: 'Procesamiento de información',
  dsCommunication: 'Comunicación',
  dsContentCreation: 'Creación de contenidos',
  dsSafety: 'Seguridad',
  dsProblemSolving: 'Resolución de problemas',
  jobType: 'Tipo de empleo',
  reimportMergeButton: 'Añadir a mi perfil',
  moreCount: 'más',
  nothingYet: 'Aquí aún no hay nada',
  oneCredit: '1 crédito',
  segProfile: 'Perfil',
  segUnanswered: 'Sin responder',
  newRole: 'Nuevo puesto',
  newEducation: 'Nueva formación',
  addRoleShort: 'Añadir un puesto',
  addEducationShort: 'Añadir formación',
  removeRoleTitle: '¿Eliminar este puesto?',
  removeEducationTitle: '¿Eliminar esta formación?',
  removeWarning: 'No se puede deshacer — tendrías que escribirlo todo otra vez. Los CV ya creados conservan lo que se usó para hacerlos.',
  addHighlight: 'Añadir un logro',
  addLanguage: 'Añadir un idioma',
  addCertification: 'Añadir certificación',
  removeItem: 'Quitar',
  skillPlaceholder: 'Escribe una aptitud y pulsa Enter',
  industryPlaceholder: 'Escribe un sector y pulsa Enter',
  techPlaceholder: 'Escribe una herramienta y pulsa Enter',
  highlightPlaceholder: 'Qué hiciste y qué cambió por ello',
  languageName: 'Idioma',
  certName: 'Certificación',
  issuer: 'Emitida por',
  yearLabel: 'Año',
  clearLink: 'Borrar',
  lvlElementary: 'Básico',
  lvlLimited: 'Conversacional',
  lvlProfessional: 'Profesional',
  lvlFull: 'Fluido',
  lvlNative: 'Nativo',
  photo: "Foto",
  addPhoto: "Añadir foto",
  changePhoto: "Cambiar foto",
  photoHint: "Para CV Europass y continentales.",
}
