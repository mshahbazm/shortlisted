import type { tLocale } from '../en'

export const onboarding: tLocale['onboarding'] = {
  // No-CV guided builder
  buildTitle: 'Creiamo il tuo profilo insieme.',
  buildLead: 'Scegli a che punto sei e le domande si adattano.',
  buildStartingTitle: 'Sto iniziando ora',
  buildStartingSub: 'Studio, o cerco il primo lavoro.',
  buildWorkingTitle: 'Ho già lavorato',
  buildWorkingSub: 'Solo che non ho mai avuto bisogno di un CV.',
  buildHaveResumeTitle: 'Ho già un curriculum',
  buildHaveResumeSub: 'Caricalo e lo trasformiamo nel tuo profilo.',
  buildCv: 'Crea il mio CV',
  probeNext: 'Avanti',
  probeTitle: 'Un paio di domande veloci.',
  probeLead: 'Rispondi a quello che puoi: rendono il CV più forte. Salta quelle che non ti riguardano.',
  talkStartingTitle: 'Cosa hai fatto?',
  talkStartingLead:
    'Un progetto, volontariato, un club, un lavoro part-time: tutto conta. Scrivi tutto qui e diventa il tuo CV.',
  talkStartingPlaceholder:
    "es. Progetto di tesi: ho creato un'app per trovare aule studio libere nel campus. Ho scritto il backend e gestito i nostri standup settimanali.",
  talkWorkingTitle: 'Il tuo lavoro più recente.',
  talkWorkingLead:
    'Dove hai lavorato, cosa facevi, di cosa vai fiero: scrivi liberamente e diventa il tuo CV.',
  talkWorkingPlaceholder:
    'es. Ho guidato un team di 6 persone alla Kordo Logistics per 3 anni. Ho rivisto i turni del weekend per non restare mai a corto di personale e formato i nuovi arrivati.',
  talkCountNeed: (n: number, min: number) => `${n} / ${min} caratteri — ancora un po'`,
  talkCountReady: 'Più condividi, migliore sarà il tuo CV — progetti, successi, tutto.',
  back: 'Indietro',
  skip: 'Salta',
  nameTitle: "Piacere di conoscerti.",
  nameLead:
    "Il tuo nome finisce sul CV. La tua email serve a dirti cosa c’è di nuovo — e ogni tanto a chiederti come va.",
  emailLabel: "Email",
  emailPlaceholder: "tu@esempio.com",
  emailWhy:
    "Novità e qualche domanda ogni tanto, nient’altro. Non è un account: non c’è nulla in cui accedere, e il tuo CV e le tue risposte restano su questo computer.",
  emailSkip: "Continua senza",
  emailInvalid: "Non sembra un indirizzo email — controllalo, oppure continua senza.",

  welcomeTitle: 'Facciamoci notare.',
  welcomeLead:
    "Crea il tuo profilo professionale una volta e Shortlisted riusa i tuoi dati e le tue risposte in ogni candidatura — controlli tutto prima di inviare. Il tuo CV, le tue risposte e la tua storia lavorativa restano su questo computer.",
  importCvTitle: 'Ho un CV',
  importCvSub: 'Carica o incolla il tuo CV — l\'IA lo trasforma nel tuo profilo. ~1 minuto.',
  startBlankTitle: 'Non ho un CV',
  startBlankSub: 'Nessun problema — dentro ti aiutiamo a creare il tuo profilo e un CV curato.',

  pasteTitle: 'Il tuo CV, per favore.',
  pasteLead: 'Carica il PDF o incolla il testo.',
  uploadPdf: 'Carica PDF',
  uploadSubIdle: 'L\'IA lo legge e crea il tuo profilo.',
  readingCv: 'Sto leggendo il tuo CV…',
  readingCloudSub: 'Sto estraendo il tuo profilo — un attimo.',
  buildingTitle: 'Sto preparando il tuo profilo…',
  buildingLead: 'Quasi fatto — pochi secondi.',
  pastePlaceholder: '…oppure incolla qui il testo del tuo CV.',
  buildProfile: 'Crea il mio profilo',
  reviewTitle: 'È tutto giusto?',
  reviewLead: (roles: number, skills: number) =>
    `Trovat${roles === 1 ? 'o 1 ruolo' : `i ${roles} ruoli`} e ${skills} competenze. Correggi quello che non torna — il resto lo modifichi quando vuoi.`,
  looksRight: 'Tutto giusto',
  linkedin: 'LinkedIn',
  github: 'GitHub',
  portfolio: 'Portfolio',
  linkedinPlaceholder: 'linkedin.com/in/you',
  githubPlaceholder: 'github.com/you',
  portfolioPlaceholder: 'yoursite.com',

  answersTitle: 'Due informazioni rapide.',
  jobTypeLabel: 'Cosa cerchi?',
  jobTypeFullTime: 'Tempo pieno',
  jobTypePartTime: 'Part-time',
  jobTypeContract: 'Contratto',
  jobTypeInternship: 'Tirocinio',
  jobTypeFreelance: 'Freelance',
  jobTypeOpenToAny: 'Aperto a tutto',
  answersLead: 'Rispondi una volta qui, mai più in una candidatura.',
  salaryHourlyLabel: 'Tariffa oraria',
  salaryHourlyPlaceholder: 'es. 25',
  salaryMonthlyLabel: 'Stipendio mensile',
  salaryMonthlyPlaceholder: 'es. 4000',
  noticeDaysLabel: 'Quando puoi iniziare? (giorni)',
  noticeDaysHint: '0 = subito',
  continue: 'Continua',

  checking: 'Un attimo…',
}
